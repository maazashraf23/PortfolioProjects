...
/*
1st Query for Tableau

*/
Create Table #DeathPercentage1(
total_cases numeric,
total_deaths numeric)
INSERT INTO #DeathPercentage1
...
FROM #DeathPercentage1

/*
2nd Query for Tableau
*/
SELECT location, MAX(cast(total_deaths as int)) as total_deaths 
FROM PortfolioProject..CovidDeaths
WHERE Continent is null
and location not in('European Union','High income','Upper middle income','Low income','World','Lower middle income')
group by location
...
*/
SELECT location, Max(population) as Population, MAX(cast(total_cases as int)) as HighestInfectionCount, (Max(cast(total_cases as int))/Max(Population))*100 as PercentPopulationInfected
FROM PortfolioProject..CovidDeaths
WHERE continent is not null
group by location
...
*/
SELECT location,date, Max(population), MAX(total_cases) as HighestInfectionCount, (Max(cast(total_cases as int))/Max(Population))*100 as PercentPopulationInfected
FROM PortfolioProject..CovidDeaths
WHERE continent is not null
...
SELECT DISTINCT Location, Date, population, MAX(cast(total_cases as int)) as InfectedPeople, (MAX(cast(total_cases as int))/population)*100 as PercentPeopleInfected
FROM PortfolioProject..CovidDeaths
WHERE continent is not null
group by location, Date, population
order by location,PercentPeopleInfected desc
...
>>>>>>> face7eb2ccbd239a5cc667e584c071f55b5f7022
